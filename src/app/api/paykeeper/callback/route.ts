...
function normalizeSum(value: string | undefined) {
...
async function readCallbackParams(request: Request): Promise<CallbackParams> {
...
async function readPaymentsFile(filePath: string): Promise<PaymentsStorage> {
  try {
    const raw = await readFile(filePath, "utf8");
    const parsed = JSON.parse(raw) as Partial<PaymentsStorage>;

    return Array.isArray(parsed.payments) ? { payments: parsed.payments } : { payments: [] };
  } catch {
    return { payments: [] };
  }
}

async function savePayment(payment: StoredPayment) {
