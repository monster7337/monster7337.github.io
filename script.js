...
if (sendToBotBtn) {
  sendToBotBtn.addEventListener('click', () => {
    ...
    if (!endAt) {
      alert('Укажи дату и время окончания розыгрыша.');
      return;
    }

    if (!channels.length) {
      alert('Добавь хотя бы один канал или чат для подписки в блоке условий.');
      return;
    }
  ...
  .filter(Boolean);
    ...
    if (mode === 'rigged-demo' && manualWinners.length === 0) {
      alert('В учебном режиме "подкрутки" укажи хотя бы одного ручного победителя.');
      return;
    }
